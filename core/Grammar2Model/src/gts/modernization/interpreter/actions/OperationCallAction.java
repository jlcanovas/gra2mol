...
public class OperationCallAction extends RuleExecutorAction {

	public OperationCallAction(ModelBuilder modelBuilder, Model model, ModelElement modelElement, HashMap<String, List<Element>> queries, Element actualNode, Rule rule, InitUnitElement iu, Gra2MoLInterpreter interpreter) {
		super(modelBuilder, model, modelElement, queries, actualNode, rule, iu, interpreter);
	}

	public void execute() {
		Gra2MoLLogger.getInstance().print("- CALL RULE: ");
		// Extract the result of the query
		List<Element> result = extractResult(initValueElement.getExtension());
		...
		if(result != null && result.size() > 0) {
			...
			while(it.hasNext()) {
				...
				Gra2MoLLogger.getInstance().print("(" + ++counter + "/" + result.size() + ") result node: " + node.getKind() + " (kind) ");

				// Locating the rule to execute
				Rule resolvedRule = locateRule(initValueElement.getValue()); 
