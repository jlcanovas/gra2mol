...
public class QueryAccessAction extends Gra2MoLInterpreterAction {

	public QueryAccessAction(ModelBuilder modelBuilder, Model model, ModelElement modelElement, HashMap<String, List<Element>> queries, Element actualNode, Rule rule, InitUnitElement iu) {
		...
		if(result != null && result.size() > 0) {
			...
			for(int i = 0; i < result.size(); i++) {
				...
				if (elem instanceof Node) {
					...
					if(position == -1) {
						// Several leaves. Extract each one and initialize the metaelement attribute
						List<Leaf> lleaf = elemNode.getAllLeaves(initValueElement.getExtension());
						Iterator<Leaf> itLeaf = lleaf.iterator();
						while(itLeaf.hasNext()) {
							Leaf l = itLeaf.next();
							setValueModelElement(l.getValue());
						}						
					} else {
						// Only one leaf. Extract the leaf
